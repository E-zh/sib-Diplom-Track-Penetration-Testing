...
### 3.1. Тестирование 92.51.39.106:8050
...
File Inclusion - нарушает способ загрузки кода приложением для выполнения.
...
### 3.2. Тестирование 92.51.39.106:7788
...
### 4.1. Подтверждения успешности атак

В результате выполнения этапов 2 (Scanning) и 3 (Testing), на сервере **92.51.39.106** были выявлены и подвергнуты эксплуатации
следующие уязвимости:

#### SQL Injection

В данном случае позволяет обойти систему авторизации на обоих сайтах сервера. Ниже приведены скриншоты эксплуатации уязвимости:

* по адресу http://92.51.39.106:8050/users/login.php расположена форма авторизации. В поле Username вводим `admin' or '1'='1'#`, пароль - любые символы:

  ![](assets/3-testing/8050/3-8050-login-success.jpg)

* по адресу http://92.51.39.106:7788/login.html расположена форма авторизации. В поле Username вводим `admin' or '1'='1--`, пароль - любые символы:
...
* по адресу http://92.51.39.106:8050/guestbook.php располагается форма отправки комментария. Введем в поле комментария код `<script>alert('Cross Site Scripting (DOM Based)')</script>">`:

  ![](assets/3-testing/8050/3-8050-css-dom-based-success.jpg)

* на сайте http://92.51.39.106:7788 данная уязвимость была автоматически найдена при сканировании ZAP. сформирована ссылка http://92.51.39.106:7788/search?q=%3Cscript%3Ealert(5397)%3C/script%3E, перейдя по которой мы увидим вредоносное сообщение:

  ![](assets/3-testing/7788/3-778-css-dom-success.jpg)

#### Cross Site Scripting (Reflected)

На сайтах есть форма поиска. Злоумышленник может ей воспользоваться для запуска вредоносного Javascript кода:

* на сайте http://92.51.39.106:8050/ введем в строку поиска код - `<a href="http://92.51.39.106:8050/" onclick="alert('Cross Site Scripting (Reflected) example!'); return false;">Ссылка на сайт NetologyVulnApp</a>` и нажмем кнопку `Search`. После выполнения поиска появится ссылка, кликнув на которую получим сообщение в браузере:

  ![](assets/3-testing/8050/3-8050-css-reflected-success.jpg)

* на сайте http://92.51.39.106:7788/ введем в строку поиска код - `<a href="http://92.51.39.106:7788/" onclick="alert('Cross Site Scripting (Reflected) example!'); return false;">Ссылка на сайт Beemers</a>` и нажмем иконку поиска или клавишу `Enter` на клавиатуре. После выполнения поиска появится ссылка, кликнув на которую получим сообщение в браузере:

  ![](assets/3-testing/7788/3-7788-css-reflected-success.jpg)

#### Cross Site Scripting (Persistent)

На http://92.51.39.106:8050/ сайте имеется форма отправки комментариев при просмотре фото (например по адресу http://92.51.39.106:8050/pictures/view.php?picid=15).
Злоумышленник может ей воспользоваться для запуска вредоносного Javascript кода. Для этого введем в форму отправки комментария код `<script>alert('Это вредоносный XSS!');</script>`
(не нужно было использовать русский текст, но не суть, код выполняется):
...
File Inclusion - нарушает способ загрузки кода приложением для выполнения.
Успешная эксплуатация уязвимости включения файлов приведет к удаленному выполнению кода на веб-сервере,
на котором работает уязвимое веб-приложение. Злоумышленник может использовать удаленное выполнение кода для
создания веб-шелла на веб-сервере, который может быть использован для порчи веб-сайта.

На сайте http://92.51.39.106:8050 имеется форма загрузки файлов, через которую можно загрузить PHP-файл [command.php](assets/3-testing/php/command.php) и выполнить его.
Без проблем удалось выполнить команду `cat /etc/passwd` и увидеть содержимое файла `passwd`:
...
Path Traversal - тип атаки на безопасность веб-приложений, при которой злоумышленник пытается получить несанкционированный
доступ к файловой системе сервера, обходя ограничения доступа и перемещаясь по файловой структуре.
Целью атаки является обычно доступ к файлам или директориям, к которым злоумышленник не имеет прав доступа.

Данная уязвимость была найдена и протестирована автоматически при сканировании ZAP сайта http://92.51.39.106:7788,
в результате была получена ссылка на чтение файла `/etc/passwd` - 
...
Remote OS Command Injection - тип атаки на безопасность веб-приложений, при которой злоумышленник вводит и выполняет
команды на удаленном сервере через уязвимость в веб-приложении, что может привести к компрометации безопасности,
утечке конфиденциальной информации и нежелательным действиям на сервере.

Уязвимость была протестирована автоматически во время сканирования ZAP сайта http://92.51.39.106:7788.
Для эксплуатации необходимо перейти по ссылке http://92.51.39.106:7788/server.html, и в поле ввода данных ввести - `127.0.0.1&cat /etc/passwd&`
и нажать кнопку Check. В результате мы получаем содержимое файла `/etc/passwd`:

![](assets/3-testing/7788/3-7788-remote-OS-command-injection-success.jpg)

### 4.2. Рекомендации по исправлению уязвимостей
...
* Параметризованные запросы (Подготовленные выражения) - вместо того, чтобы напрямую вставлять пользовательский ввод в SQL-запрос,
  используйте placeholders (маркеры) и передавайте данные отдельно. СУБД обрабатывает данные как данные, а не как код.
* Валидация и санирование входных данных:
  * Валидация: убедитесь, что введенные данные соответствуют ожидаемому типу (число, строка, дата и т.д.) и формату (например, номер телефона, email).
  * Санирование: экранируйте специальные символы, такие как кавычки, которые могут быть использованы для вставки вредоносного кода.
* Ограничение прав доступа:
  * Создайте отдельного пользователя базы данных с минимальными необходимыми правами для приложения.
  * Избегайте предоставления пользователю базы данных прав на выполнение административных операций, таких как создание таблиц или изменение схемы.
* Обновление ПО: регулярно обновляйте СУБД и используемые библиотеки/фреймворки для устранения известных уязвимостей.
* Веб-приложение-файрвол (WAF): WAF может помочь обнаружить и заблокировать вредоносные запросы.
...
Для устранения уязвимостей Cross-Site Scripting (XSS) необходимо использовать комплексный подход:

* Валидация и очистка входных данных:
  * Всегда проверяйте и очищайте данные, вводимые пользователем, на стороне сервера.
  * Используйте библиотеки для очистки XSS, такие, как `DOMPurify`, которые могут помочь в удалении вредоносных скриптов.
  * Рассматривайте все пользовательские данные как ненадежные и применяйте соответствующие меры предосторожности. 
* Безопасное кодирование выходных данных:
  * Кодируйте данные в зависимости от контекста, в котором они будут отображаться на странице (HTML, JavaScript, URL).
  * Для кодирования используйте соответствующие функции, предоставляемые фреймворком или библиотекой.
  * Избегайте использования `innerHTML` для вставки данных, так как это может привести к выполнению вредоносного кода.
  * Вместо `innerHTML` используйте `innerText` или другие безопасные методы.
* Content Security Policy (CSP):
  * Внедрите CSP для контроля источников, из которых браузер может загружать скрипты и другие ресурсы.
  * CSP позволяет предотвратить выполнение скриптов из ненадежных источников и повысить безопасность вашего приложения.
* Безопасные API и библиотеки:
  * Используйте безопасные API, предоставляемые вашим фреймворком или библиотеками, для работы с данными и DOM.
  * Избегайте использования небезопасных функций, таких как `eval()`, `setTimeout()`, `setInterval()` и `execScript()`.
* Тестирование на уязвимости:
  * Регулярно проводите тестирование на уязвимости, чтобы выявить и устранить потенциальные XSS-уязвимости.
  * Используйте автоматические сканеры веб-уязвимостей и привлекайте профессиональных тестировщиков безопасности.
* Дополнительные меры:
  * Избегайте внедрения пользовательских данных непосредственно в атрибуты HTML, если это возможно.
  * Если необходимо внедрять данные в атрибуты, убедитесь, что они правильно закодированы.
  * Используйте HTTP-only cookies и избегайте передачи конфиденциальных данных через URL.
...
Чтобы предотвратить эту уязвимость, необходимо придерживаться следующих рекомендаций:

* Проверка входных данных:
  * Обязательно проверяйте и очищайте все пользовательские данные, которые используются для формирования путей к файлам.
  * Отклоняйте входные данные, содержащие специальные символы, такие как `../`, `..\\` или другие, которые могут указывать на обход каталогов.
  * Используйте белые списки допустимых символов, а не черные списки запрещенных.
* Минимизация прав доступа:
  * Ограничьте права доступа пользователей и учетных записей, используемых приложением для доступа к файловой системе. 
  * Предоставляйте только минимально необходимые права для выполнения операций с файлами.
* Использование проверенных библиотек: при работе с файлами используйте проверенные и безопасные библиотеки, которые могут
  автоматически обрабатывать проверку входных данных и предотвращать обход пути.
* Регулярно обновляйте ваше приложение и используемые библиотеки до последних версий, чтобы устранить известные уязвимости, включая Path Traversal.
* Проводите регулярное тестирование на безопасность, включая тесты на обход пути, чтобы выявить и устранить уязвимости до того, как они будут использованы злоумышленниками. 
* Внедрите механизмы мониторинга и логирования для отслеживания попыток несанкционированного доступа к файловой системе и выявления подозрительной активности.
...
Для устранения данной уязвимости, необходимо придерживаться следующих рекомендаций:

* Всегда используйте параметризованные запросы при выполнении операций с базой данных или операционной системой.
  Это поможет избежать возможности внедрения вредоносного кода через пользовательский ввод.
...
* Предоставляйте только минимальные права доступа приложению и пользователям к системным ресурсам.
  Ограничьте доступ к системным командам и функциям, которые не являются необходимыми для работы приложения.
* Регулярно обновляйте и анализируйте код приложения на предмет уязвимостей Remote OS Command Injection.
* Используйте белый список (whitelist) разрешенных команд и операций, которые могут быть выполнены из приложения.
  Это поможет ограничить возможности злоумышленников.
...
Защита от атак Source Code Disclosure и File Inclusion включает в себя применение различных мер на уровне приложения, сервера и кода:
* Использование относительных путей к файлам и каталогам.
* Ограничение доступа к файлам.
* Фильтрация и валидация пользовательского ввода.

Следование вышеуказанным рекомендациям поможет защитить приложение и снизить риск успешных атак на веб-приложения, находящиеся
на тестируемом сервере **92.51.39.106**.
